    ...
    if (!!token && !!data.id) {
            ...
            function (err, userToken) {
              if (!!userToken) {
                if (userToken.lock === true) {
                  callback(
                    { title: "CẤM", text: "Tài khoản bị vô hiệu hóa." },
                    false
                  );
                  return void 0;
                }
                ...
                if (void 0 !== userToken.fail && userToken.fail > 3) {
                  callback(
                    { title: "THÔNG BÁO", text: "Vui lòng đăng nhập !!" },
                    false
                  );
                  ...
                  if (userToken.local.token === token) {
                    ...
                    callback(
                      {
                        title: "THẤT BẠI",
                        text: "Bạn hoặc ai đó đã đăng nhập trên 1 thiết bị khác !!",
                      },
                      false
                    );
                  }
                ...
                callback(
                  { title: "THẤT BẠI", text: "Truy cập bị từ chối !!" },
                  false
                );
              }
            }
      ...
      if (!validator.isLength(username, { min: 3, max: 32 })) {
        ...
        callback(
          {
            title: register ? "ĐĂNG KÝ" : "ĐĂNG NHẬP",
            text: "Tên đăng nhập chỉ gồm kí tự và số !!",
          },
          false
        );
        ...
        callback(
          {
            title: register ? "ĐĂNG KÝ" : "ĐĂNG NHẬP",
            text: "Tài khoản không được trùng với mật khẩu!!",
          },
          false
        );
      } else {
        try {
          ...
          if (register) {
            if (false) {
              //!captcha || !client.c_captcha
              client.c_captcha("signUp");
              callback(
                { title: "ĐĂNG KÝ", text: "Captcha không tồn tại." },
                false
              );
            } else {
              ...
              if (checkCaptcha) {
                User.findOne({ "local.username": username }).exec(function (
                  ...
                  if (!!check) {
                    client.c_captcha("signUp");
                    callback(
                      { title: "ĐĂNG KÝ", text: "Tên tài khoản đã tồn tại !!" },
                      false
                    );
                  } else {
                    User.create(
                ...
                client.c_captcha("signUp");
                callback(
                  { title: "ĐĂNG KÝ", text: "Captcha không đúng." },
                  false
                );
              }
            }
            ...
            User.findOne({ "local.username": username }, function (err, user) {
              if (user) {
                UserInfo.findOne(
                  ...
                  function (err, userI) {
                    if (userI != null) {
                      if (userI.type === true) {
                        callback(
                          { title: "CẤM", text: "Truy cập bị từ chối !!" },
                          false
                        );
                        return void 0;
                      } else {
                        ...
                        if (void 0 !== user.fail && user.fail > 3) {
                          if (!captcha || !client.c_captcha) {
                            ...
                            callback(
                              {
                                title: "ĐĂNG NHẬP",
                                text: "Phát hiện truy cập trái phép, vui lòng nhập captcha để tiếp tục.",
                              },
                              false
                            );
                          } else {
                          ...
                          if (user.validPassword(password)) {
                            if (!user.local.ban_login) {
                              ...
                              callback(
                                {
                                  title: "ĐĂNG NHẬP",
                                  text: "Tài khoản bị khoá. Vui lòng liên hệ CSKH để được hỗ trợ",
                                },
                                false
                              );
                            }
                ...
                callback(
                  { title: "ĐĂNG NHẬP", text: "Tên Tài Khoản không tồn tại!!" },
                  false
                );
              }
            });
